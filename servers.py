...
def load_servers():
    global servers
    try:
        with open(config["data_file"], "r", encoding="utf-8") as file:  # ✅ "r" — читаем
            servers = json.load(file)  # ✅ Загружаем данные В переменную
        print("✅ Данные загружены из", config["data_file"])
    except FileNotFoundError:
        print("⚠️ Файл", config["data_file"], "не найден. Начинаем с пустого списка.")
        servers = []
...
def save_servers():
    ...
    with open(config["data_file"], "w", encoding="utf-8") as file:
        # Сохраняем список servers в формате JSON
